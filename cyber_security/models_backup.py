...
class SecurityConfiguration(models.Model):
    ...
    config_id = models.CharField(max_length=100, unique=True, verbose_name="معرف الإعداد")
    name = models.CharField(max_length=200, verbose_name="اسم الإعداد")
    description = models.TextField(verbose_name="وصف الإعداد")
    
    # تصنيف الإعداد
    category = models.CharField(max_length=20, choices=CONFIG_CATEGORIES,
                              verbose_name="فئة الإعداد")
    config_type = models.CharField(max_length=10, choices=CONFIG_TYPES,
                                 verbose_name="نوع الإعداد")
    
    # القيم
    default_value = models.JSONField(verbose_name="القيمة الافتراضية")
    current_value = models.JSONField(verbose_name="القيمة الحالية")
    allowed_values = models.JSONField(default=list, blank=True,
                                    verbose_name="القيم المسموحة")
    
    # البيئة والنطاق
    environment = models.CharField(max_length=15, choices=ENVIRONMENT_TYPES,
                                 default='ALL', verbose_name="البيئة")
    scope = models.JSONField(default=dict, verbose_name="النطاق")
    
    # إعدادات التحقق
    validation_rules = models.JSONField(default=dict, verbose_name="قواعد التحقق")
    min_value = models.JSONField(null=True, blank=True, verbose_name="الحد الأدنى")
    max_value = models.JSONField(null=True, blank=True, verbose_name="الحد الأقصى")
    ...
    requires_restart = models.BooleanField(default=False, verbose_name="يتطلب إعادة تشغيل")
    
    # الأذونات
    can_be_modified = models.BooleanField(default=True, verbose_name="قابل للتعديل")
    modification_requires_approval = models.BooleanField(default=False,
                                                       verbose_name="التعديل يتطلب موافقة")
    authorized_roles = models.JSONField(default=list, verbose_name="الأدوار المخولة")
    
    # تتبع التغييرات
    last_modified_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True,
                                       related_name='modified_security_configs',
                                       verbose_name="آخر تعديل بواسطة")
    last_modified_at = models.DateTimeField(auto_now=True, verbose_name="تاريخ آخر تعديل")
    change_reason = models.TextField(blank=True, verbose_name="سبب التغيير")
    
    # المراجعة والاعتماد
    approved_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True,
                                  related_name='approved_security_configs',
                                  verbose_name="مُعتمد من")
    approved_at = models.DateTimeField(null=True, blank=True, verbose_name="تاريخ الاعتماد")
    approval_notes = models.TextField(blank=True, verbose_name="ملاحظات الاعتماد")
    
    # سجل التغييرات
    change_history = models.JSONField(default=list, verbose_name="سجل التغييرات")
    
    # التنبيهات والمراقبة
    alert_on_change = models.BooleanField(default=False, verbose_name="تنبيه عند التغيير")
    monitor_compliance = models.BooleanField(default=False, verbose_name="مراقبة الامتثال")
    compliance_rules = models.JSONField(default=dict, verbose_name="قواعد الامتثال")
    
    # التوثيق
    documentation_url = models.URLField(blank=True, verbose_name="رابط التوثيق")
    examples = models.JSONField(default=list, verbose_name="أمثلة")
    
    # معلومات إضافية
    tags = models.JSONField(default=list, verbose_name="العلامات")
    related_configs = models.JSONField(default=list, verbose_name="الإعدادات المرتبطة")
    ...
    def needs_approval(self):
        """يحتاج لموافقة"""
        return (self.modification_requires_approval and 
                not self.approved_at and 
                not self.is_default)
    
    def validate_value(self, value):
        """التحقق من صحة القيمة"""
        # التحقق من النوع
        if self.config_type == 'BOOLEAN' and not isinstance(value, bool):
            return False, "القيمة يجب أن تكون منطقية (True/False)"
        
        if self.config_type == 'INTEGER' and not isinstance(value, int):
            return False, "القيمة يجب أن تكون عددًا صحيحًا"
        
        if self.config_type == 'FLOAT' and not isinstance(value, (int, float)):
            return False, "القيمة يجب أن تكون عددًا"
        
        if self.config_type == 'STRING' and not isinstance(value, str):
            return False, "القيمة يجب أن تكون نصًا"
        
        # التحقق من القيم المسموحة
        if self.allowed_values and value not in self.allowed_values:
            return False, f"القيمة غير مسموحة. القيم المسموحة: {self.allowed_values}"
        
        # التحقق من الحدود
        if self.min_value is not None and value < self.min_value:
            return False, f"القيمة أقل من الحد الأدنى {self.min_value}"
        
        if self.max_value is not None and value > self.max_value:
            return False, f"القيمة أكبر من الحد الأقصى {self.max_value}"
        
        return True, "القيمة صحيحة"
    
    def update_value(self, new_value, user=None, reason=""):
        """تحديث القيمة مع تسجيل التغيير"""
        # التحقق من صحة القيمة
        is_valid, message = self.validate_value(new_value)
        if not is_valid:
        ...
        # إعادة تعيين الموافقة إذا كانت مطلوبة
        if self.modification_requires_approval:
    ...
    def reset_to_default(self, user=None, reason="إعادة تعيين للقيمة الافتراضية"):
        """إعادة تعيين للقيمة الافتراضية"""
        self.update_value(self.default_value, user, reason)
